...
static void import_scene(std::vector<scene_mesh_subset_data> &out_mesh_subset_data, std::vector<std::string> const &material_filter_names, char const *path);

static constexpr uint32_t const baked_texture_width = 4096U;
static constexpr uint32_t const baked_texture_height = 4096U;
...
void Demo::Init(ID3D11Device *device, ID3D11DeviceContext *device_context)
{
	...
	std::string const file_name = "keqing.gltf";
	// 11
	std::vector<std::string> const material_filter_names = {"face", "eyebrow", "skin", "eye", "nail"};
	// 5
	// std::vector<std::string> const material_filter_names = { "stocking-1",  "stocking-2", "stocking-3", "stocking-4", "shoe" };
	// 19
	// std::vector<std::string> const material_filter_names = { "clothing",  "necklace-1", "necklace-2", "necklace-3", "brooch-1", "brooch-2", "brooch-3", "brooch-4", "brooch-5", "brooch-6", "brooch-7", "brooch-8", "beading-1", "beading-2", "beading-3", "beading-4", "beading-5", "beading-6"};
	// 12
	// std::vector<std::string> const material_filter_names = { "bodice-1",  "bodice-2", "bodice-3", "bodice-4", "bodice-5", "belt-1", "belt-2", "belt-3", "sleeve-1", "sleeve-2"};
	// 10
	// std::vector<std::string> const material_filter_names = {"hat", "skirt-1", "skirt-2", "skirt-3", "skirt-4", "skirt-5", "skirt-6", "skirt-7", "skirt-8", "skirt-9"};
	// 2
	// std::vector<std::string> const material_filter_names = {"hair-1", "hair-2"};
	std::vector<scene_mesh_subset_data> mesh_subset_data;
	import_scene(mesh_subset_data, material_filter_names, file_name.c_str());
	...
	for (size_t mesh_subset_index = 0U; mesh_subset_index < mesh_subset_data.size(); ++mesh_subset_index)
	{
		...
		{
			// TODO: remove metallic
			DirectX::XMFLOAT4 base_color_data(in_mesh_subset_data.m_base_color_factor.x * (1.0F - in_mesh_subset_data.m_metallic_factor), in_mesh_subset_data.m_base_color_factor.y * (1.0F - in_mesh_subset_data.m_metallic_factor), in_mesh_subset_data.m_base_color_factor.z * (1.0F - in_mesh_subset_data.m_metallic_factor), 1.0F);
			...
			{
				{
					...
					base_color_image_asset_file_name_fake = (base_color_image_asset_file_name + ".fake");
				}
			}
	...
	this->m_scene_textures.reserve(this->m_scene_textures.size() + mapped_textures.size());
...
#include "_internal_baking_vertex.inl"
	...
	this->m_baking_rs = NULL;
	{
		...
		rasterizer_desc.MultisampleEnable = FALSE;
		HRESULT res_device_create_rasterizer_state = device->CreateRasterizerState(&rasterizer_desc, &this->m_baking_rs);
		assert(SUCCEEDED(res_device_create_rasterizer_state));
	}

	this->m_baking_fs = NULL;
	{
		static
#include "_internal_baking_fragment.inl"
			HRESULT res_device_create_pixel_shader = device->CreatePixelShader(baking_fragment_shader_module_code, sizeof(baking_fragment_shader_module_code), NULL, &this->m_baking_fs);
...
#include "_internal_full_screen_vertex.inl"
	...
	this->m_full_screen_fs = NULL;
	{
		static
#include "_internal_full_screen_fragment.inl"
			HRESULT res_device_create_pixel_shader = device->CreatePixelShader(full_screen_fragment_shader_module_code, sizeof(full_screen_fragment_shader_module_code), NULL, &this->m_full_screen_fs);
		assert(SUCCEEDED(res_device_create_pixel_shader));
	}
}
...
void Demo::Tick(ID3D11Device *device, ID3D11DeviceContext *device_context, ID3D11RenderTargetView *swap_chain_back_buffer_rtv, uint32_t swap_chain_image_width, uint32_t swap_chain_image_height)
{
	// Baking
	{
		ID3D11RenderTargetView *const bind_rtvs[1] = {this->m_baked_base_color_rtv};
		device_context->OMSetRenderTargets(sizeof(bind_rtvs) / sizeof(bind_rtvs[0]), bind_rtvs, NULL);

		float const middle_grey_gamma = static_cast<float>(std::pow(0.18, 2.2));
		FLOAT color_rgba[4] = {middle_grey_gamma, middle_grey_gamma, middle_grey_gamma, 0.0F};
		device_context->ClearRenderTargetView(this->m_baked_base_color_rtv, color_rgba);
		...
		device_context->PSSetShader(this->m_baking_fs, NULL, 0U);
		...
		for (size_t mesh_subset_index = 0U; mesh_subset_index < this->m_scene_mesh_subsets.size(); ++mesh_subset_index)
		{
			...
			ID3D11ShaderResourceView *const bind_ps_srvs[1] = {scene_mesh_subset.m_base_color_texture_view};
			...
			ID3D11ShaderResourceView *const unbind_ps_srvs[1] = {NULL};
	...
	// Full Screen
	{
		...
		device_context->PSSetShader(this->m_full_screen_fs, NULL, 0U);

		ID3D11ShaderResourceView *const bind_srvs[1] = {this->m_baked_base_color_srv};
		...
		ID3D11ShaderResourceView *const unbind_srvs[1] = {NULL};
...
static void import_mesh(std::vector<scene_mesh_subset_data> &out_mesh_subset_data, std::vector<std::string> const &material_filter_names, cgltf_data const *data, cgltf_mesh const *mesh);
...
static void import_scene(std::vector<scene_mesh_subset_data> &out_mesh_subset_data, std::vector<std::string> const &material_filter_names, char const *path)
{
	...
	// TODO: we only handle the first mesh
	cgltf_mesh const *mesh = ((data->meshes_count > 0) ? (&data->meshes[0]) : NULL);
	assert(NULL != mesh);

	import_mesh(out_mesh_subset_data, material_filter_names, data, mesh);
}
...
static void import_mesh(std::vector<scene_mesh_subset_data> &out_mesh_subset_data, std::vector<std::string> const &material_filter_names, cgltf_data const *data, cgltf_mesh const *mesh)
{
	...
	size_t selected_primitive_count = 0U;
	...
	for (size_t primitive_index = 0; primitive_index < mesh->primitives_count; ++primitive_index)
	{
		...
		bool selected_material = false;
		{
			...
			for (size_t material_filter_name_index = 0U; material_filter_name_index < material_filter_names.size(); ++material_filter_name_index)
			{
				if (0 == material_filter_names[material_filter_name_index].compare(primitive_material->name))
				{
	...
	std::cout << "Selected Primitive Count:" << selected_primitive_count << std::endl;
}
...
static cgltf_result cgltf_custom_read_file(const struct cgltf_memory_options *memory_options, const struct cgltf_file_options *, const char *path, cgltf_size *size, void **data)
{
	...
	HANDLE file = CreateFileW(path_utf16, FILE_READ_DATA | FILE_READ_ATTRIBUTES, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
