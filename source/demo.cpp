...
void Demo::Init(ID3D11Device3 *device, ID3D11DeviceContext *device_context)
{
    ...
    std::cout << "TODO: At this moment, only the first mesh is considered!" << std::endl;

    std::cout << "TODO: At this moment, only the base color texture is considered!" << std::endl;
...
#include "_internal_baking_pixel.inl"
            HRESULT res_device_create_pixel_shader = device->CreatePixelShader(baking_pixel_shader_module_code, sizeof(baking_pixel_shader_module_code), NULL, &this->m_baking_fs);
...
#include "_internal_full_screen_vertex.inl"
    ...
    this->m_full_screen_clear_fs = NULL;
    {
...
#include "_internal_full_screen_clear_pixel.inl"
            HRESULT res_device_create_pixel_shader = device->CreatePixelShader(full_screen_clear_pixel_shader_module_code, sizeof(full_screen_clear_pixel_shader_module_code), NULL, &this->m_full_screen_clear_fs);
        assert(SUCCEEDED(res_device_create_pixel_shader));
    }

    this->m_full_screen_transfer_fs = NULL;
    {
        static
#include "_internal_full_screen_transfer_pixel.inl"
            HRESULT res_device_create_pixel_shader = device->CreatePixelShader(full_screen_transfer_pixel_shader_module_code, sizeof(full_screen_transfer_pixel_shader_module_code), NULL, &this->m_full_screen_transfer_fs);
        assert(SUCCEEDED(res_device_create_pixel_shader));
    }
}
...
void Demo::Tick(ID3D11Device3 *device, ID3D11DeviceContext *device_context, ID3D11RenderTargetView *swap_chain_back_buffer_rtv, uint32_t swap_chain_image_width, uint32_t swap_chain_image_height)
{
    // Baking
    {
        ...
        // Clear
        {
            ...
            device_context->PSSetShader(this->m_full_screen_clear_fs, NULL, 0U);
    ...
    // Full Screen
    {
        ...
        device_context->VSSetShader(this->m_full_screen_vs, NULL, 0U);
        device_context->PSSetShader(this->m_full_screen_transfer_fs, NULL, 0U);
...
void Demo::Destroy(ID3D11Device3 *device, ID3D11DeviceContext *device_context)
{
    ...
    this->m_full_screen_clear_fs->Release();
    this->m_full_screen_transfer_fs->Release();
}
...
static void import_scene(std::vector<scene_mesh_subset_data> &out_mesh_subset_data, char const *path)
{
    ...
    import_mesh(out_mesh_subset_data, data, mesh);
}
