...
{
    float3 packed_normal = g_normal_texture.Sample(g_sampler, in_uv).rgb;
    float3 shading_normal_tangent_space = packed_normal * 2.0 - float3(1.0, 1.0, 1.0);
    float3 base_color = g_base_color_texture.Sample(g_sampler, in_uv).rgb;
    float3 packed_metallic_roughness = g_metallic_roughness_texture.Sample(g_sampler, in_uv).rgb;
    float roughness = packed_metallic_roughness.g;
    float metallic = packed_metallic_roughness.b;

    const float3 geometry_normal_world_space = float3(0.0, 1.0, 0.0);
    const float4 tangent_world_space = float4(1.0, 0.0, 0.0, 1.0);
    float3 bitangent_world_space = cross(geometry_normal_world_space, tangent_world_space.xyz) * tangent_world_space.w;
    float3 shading_normal_world_space = normalize(tangent_world_space.xyz * shading_normal_tangent_space.x + bitangent_world_space * shading_normal_tangent_space.y + geometry_normal_world_space * shading_normal_tangent_space.z);
    ...
    const float3 N = shading_normal_world_space;
